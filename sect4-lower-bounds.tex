...
The broadcast time $\tau\left(V,\{v_1\}\right)$ is hence no less than the sum of the time $\lceil\log\beta(v_i)\rceil$ it takes to inform $v_i$,
and the subsequent time $\tau_i$ needed to inform all nodes in $T[i]$.
Consequently, if $T$ is an optimal broadcast tree,
\begin{equation}
  \tau\left(V,\{v_1\}\right)=\max\left\{\lceil\log\beta(v_i)\rceil+\tau_i: i=2,\ldots,n\right\}. \label{eq:tau}
\end{equation}
