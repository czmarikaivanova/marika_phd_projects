...
For a given maximum broadcast time $t_{\text{max}}$, we maximize the number of arcs along which a signal is relayed within $t_{\text{max}}$.
If the optimal value is $n-1$, it is clear that we found a spanning broadcast tree with broadcast time $t_{\text{max}}$.
In case the optimal value is less than $n-1$, the gien $t_{\text{max}}$ is insufficient and we have to try solving the problem with increased time limit.
...
Similarly, if an upper bound $\bar{t}$ is known and $t_{\text{max}}=\bar{t}-1$, the process can be terminated when the objective function is less then $n-1$, because $\bar{t}$ is than the optimal value.

The decision model \label{mod:basic:dec}  has the form
\begin{subequations}\label{mod:basic:dec}
\begin{align}
\label{mod:basic:dec:obj} \max \sum\limits_{(u,v) \in A}\sum\limits_{k=1}^{t_{\text{max}}}x_{u,v}^k \\ 
\notag \text{s. t. } \\
...
The objective function \eqref{mod:basic:dec:obj} maximizes the number of arcs relaying a signal.
The actual target function is the minimum time required for spreading the signal to all nodes, and so we are looking for the smallest value of $t_{text{max}}$.
